...
const Production1_skill_Matrix = () => {
  ...
  const [data4mList, setData4mList] = useState<Data4M[]>([]);
  ...
  const fetchData4m = async () => {
    ...
    const locationMap: Record<string, string> = {
      ...
      "FPCA Camera 2": "NPCT",
      "FPCA Camera 3": "AISIN",
      "FPCA Camera 4": "PANA PMFTH",
      "PCBA Camera": "Kyocera",
      "Motor Sensor": "ASIN",        // เดิม
      "ICT Heater Sensor": "PANA PMFTH", // เดิม
      "Automotive": "W/H",           // เดิม
      "Finish Good": "F/G",          // เดิม
      "Touch_up": "Touch Up",        // เดิม
    };


    finalLocation = locationMap[locationto4m] || locationto4m;

    try {
      const response = await fetch(
        `/api/4M?S_team=${selectedTeam}&locationto4m=${encodeURIComponent(finalLocation)}`
      );
      ...
      if (result && result.data) {
        setData4mList([result.data]); // ✅ แปลง object เป็น array
        setShow4MCard(true);
        setNo4mData(false);
      } else {
        setShow4MCard(false);
  ...
  return (
    <div className="flex-1 min-h-screen bg-white overflow-y-auto flex flex-col items-center justify-start h-full w-full">
      ...
      {/* 4M Data Modal */}
      {show4MCard && data4mList && data4mList.length > 0 && (
